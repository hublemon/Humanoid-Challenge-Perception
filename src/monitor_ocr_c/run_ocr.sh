...
#
#  사용법:
#    bash ~/ai_worker/monitor_ocr_c/run_ocr.sh          # 로컬 실행
#    bash ~/ai_worker/monitor_ocr_c/run_ocr.sh docker   # docker 컨테이너 실행
#
#  옵션 (환경변수로 재정의 가능):
...
if [ "$MODE" = "docker" ]; then
    ...
    docker exec -it ai_worker bash -c "
        ...
        ros2 run monitor_ocr_c monitor_ocr_c_node --ros-args \
            $MODE_ARG \
    ...
    if [ -n "$WS" ]; then
        ...
        echo "    cd ~/ros2_ws && colcon build --packages-select monitor_ocr_c"
        exit 1
    fi
    NODE=$(find ~/ros2_ws /root/ros2_ws 2>/dev/null -name "monitor_ocr_c_node" -path "*/install/*" | head -1)
    if [ -z "$NODE" ]; then
        echo "[!] monitor_ocr_c_node 실행파일을 찾을 수 없습니다."
        exit 1
    fi
    $NODE --ros-args \
        $MODE_ARG \
