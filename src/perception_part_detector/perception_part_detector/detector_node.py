...
class PerceptionDetectorNode(Node):
        ...
        if self.publish_debug_image:
            self.debug_pubs = {
                'head': self.create_publisher(
                    Image, f'/detector_debug_image/{self.part_name}/head', 10
                ),
                'wrist_left': self.create_publisher(
                    Image, f'/detector_debug_image/{self.part_name}/wrist_left', 10
                ),
                'wrist_right': self.create_publisher(
                    Image, f'/detector_debug_image/{self.part_name}/wrist_right', 10
                ),
            }
    ...
    def _default_model_path(self, pkg_dir, part_name):
        weights_dir = Path(pkg_dir) / 'weights'
        candidates = []
