...
Run the wrist pipe center node:
...
```bash
ros2 topic echo /perception/wrist/pipe_top_centers
ros2 topic hz /perception/wrist/pipe_top_centers
```

Check debug image:

```bash
rqt_image_view /perception/wrist/pipe_top_centers_debug_image
```

Alternative image checks:

```bash
ros2 topic echo /perception/wrist/pipe_top_centers_debug_image/header --once
ros2 topic hz /perception/wrist/pipe_top_centers_debug_image
```

Check required inputs:

```bash
ros2 topic echo /detections/scenario_c/pipe --once
...
```bash
ros2 param set /wrist_pipe_top_centers temporal_reject_unstable false
ros2 param set /wrist_pipe_top_centers rim_depth_band_m 0.030
ros2 param set /wrist_pipe_top_centers publish_debug_image true
```

Wrist camera 전용 패키지입니다. detector가 검증 완료해서 보낸
`pipe` detection들, 보통 4개를 받아서 각 파이프 입구의
...
RGB image plane으로 projection합니다. 그 다음 RGB 기준 `pipe` bbox/mask의
ring 영역에 들어온 depth point만 사용합니다.

## 입력 / 출력

```text
/detections/scenario_c/pipe  (perception_part_detector/msg/PartDetectionArray)
  detections[i].class_name    = "pipe"
...
/perception/wrist/pipe_top_centers  (geometry_msgs/msg/PoseArray)
  poses[0..N-1] = 각 pipe의 3D top-center
```
...
| `opening_class` | `pipe` | 처리할 입구 class 접두사 (`pipe`, `pipe_0`, `pipe_1` 등 모두 수신) |
...
1. `/detections/scenario_c/pipe` 안에 `pipe`로 시작하는 class_name (`pipe`, `pipe_0` 등) 4개가 들어오는지.
