...
## Common temporal smoothing parameters

These parameters apply to all target modes. The filter stores recent
`base_link` observations, clusters observations within a distance gate, and
publishes the median of the stable cluster.

| parameter | type | default | description |
| --- | --- | --- | --- |
| `temporal_smoothing_enable` | bool | `true` | Enable base-frame temporal stabilization for published poses. |
| `temporal_window_sec` | float | `0.60` | Time window used for recent observations. |
| `temporal_min_observations` | int | `2` | Minimum nearby observations required before publishing a stable pose. |
| `temporal_position_gate_m` | float | `0.035` | Maximum base-frame distance for observations to be considered one cluster. |
| `temporal_max_history` | int | `50` | Maximum number of observations kept in memory. |
| `temporal_reject_unstable` | bool | `true` | When true, suppress output until the cluster has enough observations. |
