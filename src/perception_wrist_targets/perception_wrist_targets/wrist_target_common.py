...
class WristTargetCenterNode(Node):
    ...
    def __init__(self, preset: TargetPreset) -> None:
        ...
        self._temporal_history = deque(maxlen=self.temporal_max_history)
        ...
        self.get_logger().info(
            ...
            f'temporal_min_obs={self.temporal_min_observations}')
    ...
    def synced_cb(self, rgb_msg, depth_msg, rgb_info, depth_info) -> None:
        ...
        temporal_result = self._temporal_smooth_base_xyz(base_xyz)
        if temporal_result is None:
            self._publish_debug(rgb_msg, rgb, bbox, aux_mask, False, 'temporal waiting', center_uv)
            return
        base_xyz, temporal_status = temporal_result
        if temporal_status:
            method = f'{method}_{temporal_status}'
    ...
    def _estimate_surface_target(
            self, det, mask, bbox, pts_color, u_proj, v_proj, K_rgb, h, w,
            use_top_percentile=False):
    ...
    def _temporal_smooth_base_xyz(self, base_xyz):
        if (not self.temporal_smoothing_enable or
                self.temporal_window_sec <= 0.0):
            return np.asarray(base_xyz, dtype=np.float64), ''

        raw = np.asarray(base_xyz, dtype=np.float64).reshape(3)
        if not np.all(np.isfinite(raw)):
            return raw, ''

        now_sec = self.get_clock().now().nanoseconds * 1e-9
        self._prune_temporal_history(now_sec)
        self._temporal_history.append((now_sec, raw.copy()))

        cluster = self._temporal_cluster_for(raw)
        if len(cluster) < self.temporal_min_observations:
            status = f'temporal_wait_{len(cluster)}of{self.temporal_min_observations}'
            if self.temporal_reject_unstable:
                self._warn(
                    f'{self.target_class} waiting for stable temporal observations '
                    f'({len(cluster)}/{self.temporal_min_observations}).',
                    2.0)
                return None
            return raw, status
        ...
        if not np.all(np.isfinite(smoothed)):
            return raw, ''
        return smoothed, f'temporal_median_n{len(cluster)}'

    def _prune_temporal_history(self, now_sec):
        while (self._temporal_history and
               now_sec - self._temporal_history[0][0] > self.temporal_window_sec):
            self._temporal_history.popleft()

    def _temporal_cluster_for(self, base_xyz):
        center = np.asarray(base_xyz, dtype=np.float64).reshape(3)
        cluster = []
        for _, observed in self._temporal_history:
            if float(np.linalg.norm(observed - center)) <= self.temporal_position_gate_m:
