...
_yolo_model = None
# YOLO warp 출력 크기 = FALLBACK bw × (bh * 1.1) 으로 ry>1 비율도 수용
    ...
    if model_path is None:
        ...
        try:
            ...
            model_path = os.path.join(
                get_package_share_directory('monitor_ocr_a'), 'best.pt')
        except Exception:
...
def find_display_yolo(img, conf_thresh=0.50, out_scale=1):
    ...
    out_scale:   출력 warp 해상도 배율 (기본 406x237). 작은 글자/숫자를 다루는
                 파이프라인(부품 수량 등)은 배율을 높여 디테일 손실을 줄인다.
    Returns: (warped_img, bx=0, by=0, bw, bh) or None."""
    ...
    warp_w, warp_h = _WARP_W * out_scale, _WARP_H * out_scale
    bw, bh = _WARP_W * out_scale, _FALLBACK_BBOX[3] * out_scale  # 비율 기준 bh 유지
    dst = np.array(
...
def ocr_all(engine, img):
    from monitor_ocr_a.paddle_compat import ocr_run
    items = []
...
def ocr_line(engine, img):
    from monitor_ocr_a.paddle_compat import ocr_run_single_line
