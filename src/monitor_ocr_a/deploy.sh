...
# ── 설정 ──────────────────────────────────────────────────────────────────────
ROBOT="${1:-robotis@ffw-SNPR48A1087.local}"
ROBOT_PW="${2:-root}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROBOT_DEST="~/ai_worker/monitor_ocr_a"
...
if ! $SSH "echo ok" &>/dev/null; then
    ...
    echo "  - 직접 IP 지정: bash deploy.sh robotis@<IP주소>"
    exit 1
fi
echo "      연결 성공"
echo ""
...
$SSH "docker exec ai_worker bash -c \"
    cp -r ~/ai_worker/monitor_ocr_a /root/ros2_ws/src/ 2>/dev/null || true
    source /opt/ros/jazzy/setup.bash &&
    cd /root/ros2_ws &&
    colcon build --packages-select monitor_ocr_a 2>&1 | tail -5
\""
echo "      완료"
...
echo "     bash ~/ai_worker/monitor_ocr_a/run_ocr.sh"
echo ""
